...
public class PersonalUserService {
    ...
    private static final Logger log = LoggerFactory.getLogger(PersonalUserService.class);
    private final PersonalUserJpaRepository boardJpaRepository;
    ...
    @Transactional
    public PersonalUser save(PersonalUserRequest.SaveDTO saveDTO, User sessionUser) {
        ...
        PersonalUser board = saveDTO.toEntity(sessionUser);
     ...
     * 게시글 목록 조회
     */
    public List<PersonalUser> findAll() {
        // 1. 로그 기록
        // 2. 데이터 베이스 게시글 조회
        // 3. 로그 기록
        // 4. 조회된 게시글 목록 반환
        log.info("게시글 조회 서비스 처리 시작");
        List<PersonalUser> boardList = boardJpaRepository.findAllJoinUser();
    ...
    public PersonalUser findById(Long id) {
        ...
        PersonalUser board = boardJpaRepository.findByIdJoinUser(id).orElseThrow(() -> {
     ...
     */
    @Transactional
    public PersonalUser updateById(Long id, PersonalUserRequest.UpdateDTO updateDTO, User sessionUser) {
        // 1. 로그 기록
        // 2. 수정할 게시글 조회
        // 3. 권한 체크
        // 4. 권한이 없다면 403 에러 처리
        ...
        PersonalUser board = boardJpaRepository.findById(id).orElseThrow(() -> {
            log.warn("게시글 조회 실패 - id {}", id);
    ...
    public void checkBoardOwner(Long boardId, Long userId) {
        PersonalUser board = findById(boardId);
