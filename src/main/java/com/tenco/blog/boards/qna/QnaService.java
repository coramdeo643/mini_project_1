...
public class QnaService {
    ...
    public Qna save(QnaRequest.SaveDTO saveDTO, User sessionUser) {
        ...
        Qna board = saveDTO.toEntity(sessionUser);
        qnaJpaRepository.save(board);
    ...
    public List<Qna> findAll() {
        ...
        List<Qna> boardList = qnaJpaRepository.findAllJoinUser();
    ...
    public Qna findById(Long id) {
        ...
        Qna board = qnaJpaRepository.findByIdJoinUser(id).orElseThrow(() -> {
     ...
     */
    @Transactional
    public Qna updateById(Long id, QnaRequest.UpdateDTO updateDTO, User sessionUser) {
        // 1. 로그 기록
        // 2. 수정할 게시글 조회
        // 3. 권한 체크
        // 4. 권한이 없다면 403 에러 처리
        ...
        Qna board = qnaJpaRepository.findById(id).orElseThrow(() -> {
            log.warn("게시글 조회 실패 - id {}", id);
